(function() {
	...
	function getMethodInfo(sClassName, sMethodName) {
		for (var i = 0, len = jindoApis.length; i < len; i++) {
			if (jindoApis[i].className.indexOf(sClassName) != -1) {
				for (var j = 0, l = jindoApis[i].methods.length; j < l; j++) {
					console.log(sMethodName + " == " + jindoApis[i].methods[j].name);
					if (jindoApis[i].methods[j].name.indexOf(sMethodName) != -1) {
						var ret = jindoApis[i].methods[j];
